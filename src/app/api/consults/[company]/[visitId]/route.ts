...
export async function GET(
  req: NextRequest,
  context: { params: Promise<{ company: string; visitId: string }> }
) {
  const { company, visitId } = await context.params;
  ...
  try {
    const consult = await getConsultByCompanyByEmployeeByVisit(
      company,
      employeeName,
      birthYear,
      visitId
